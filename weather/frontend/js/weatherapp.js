...
async function fetchWeatherImages() {
    const imageUrl = 'http://localhost/weather/frontend/js/image.json';
...
async function fetchWeather(city) {
    try {
        const response = await fetch(`http://localhost/weather/backend/weather.php?city=${city}`);
...
async function fetchWeatherByCoordinates(latitude, longitude) {
    ...
    try {
        const response = await fetch(`http://localhost/weather/backend/weather.php?lat=${latitude}&lon=${longitude}`);
        const data = await response.json();
